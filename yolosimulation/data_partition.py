...
def iid_split(images, no_of_clients, ratios):
    # Determine splits based on ratios
    ...
    for i in range(no_of_clients):
        ...
        partitions.append(images[start_idx:end_idx])
        start_idx = end_idx

    return partitions


def noniid_split(images, labels_dir, no_of_clients, class_names):
    # Map class -> images
    class_to_images = {i: [] for i in range(len(class_names))}

    for img in images:
        lbl_path = os.path.join(labels_dir, img.replace(".jpg", ".txt"))
        ...
        with open(lbl_path, "r") as f:
            ...
            class_to_images[cls_id].append(img)

    partitions = [[] for _ in range(no_of_clients)]

    for cls_id, imgs in class_to_images.items():
        client_id = cls_id % no_of_clients
        partitions[client_id].extend(imgs)

    return partitions


def create_partitions(dataset_dir, output_dir, no_of_clients, class_names, ratios, val_ratio=0.2, partition_type="iid"):
    ...
    # Partition selection
    if partition_type == "iid":
        partitions = iid_split(images, no_of_clients, ratios)
    elif partition_type == "noniid":
        partitions = noniid_split(images, labels_dir, no_of_clients, class_names)
    else:
