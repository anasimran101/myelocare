...
class CustomFedAvg(FedAvg):
    ...
    def _update_best_acc(
        ...
        if accuracy > self.best_acc_so_far:
            ...
            model.model.load_state_dict(arrays.to_torch_state_dict())  # load federated weights
            model_path = self.save_path / f"yolo_best_acc_{accuracy}_round_{current_round}.pt"
            model.save(model_path)
            
            logger.log(INFO, "💾 New best model saved to disk: %s", model_path)
        ...
        for current_round in range(1, num_rounds + 1):
            ...
            if evaluate_fn:
                ...
                if res is not None:
                    ...
                    self._update_best_acc(current_round, res["metrics/mAP50-95(B)"], arrays)
                    # Log to W&B
